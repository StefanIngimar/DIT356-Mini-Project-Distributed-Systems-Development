...
<template>
  <div>
    <button
      class="absolute left-[15px] top-[25px] z-50"
      v-if="!isLargeScreen"
      @click="toggleSidebar"
    >
    ...
    <Transition name="slide-fade">
      <div
        ...
        class="p-3 flex flex-col gap-3 w-[250px] min-h-screen h-full bg-neutral-900 absolute md:relative z-50"
        :class="{ none: !isSidebarOpen && !isLargeScreen }"
      >
        <div
          class="px-3 w-full flex flex-row justify-end"
          v-if="!isLargeScreen"
        >
        ...
        <SideNavButton href="/" name="Dashboard">
          <DashboardIcon />
        </SideNavButton>
        <SideNavButton href="/dentists/new" name="Dentists">
          <DoctorIcon />
        </SideNavButton>
        <SideNavButton href="/clinics/existing" name="Clinics">
          <ClinicIcon />
        </SideNavButton>
      </div>
    </Transition>
